...
public class CommandeService implements CommandeInterface{
    ...
    public void AjouterCommande(ArrayList<Panier> paniers ,Commande c) throws SQLException
    {
        ...
        {
            System.out.println(livreServices.idlivre(p.getTitreLiv()));
            Ligne_commande ligne_commande = new Ligne_commande(id,livreServices.idlivre(p.getTitreLiv()),p.getNbrArticle());
    ...
    public int getCommanid() throws SQLException
    {
        String request = "SELECT MAX(id) FROM commande";
        Statement stm = cnx.createStatement();
        ResultSet rst = stm.executeQuery(request);
